...
model.setParam('Heuristics', .25)       # Spend more effort on heuristic searches
...
# -------------------------------
Vmax = 1  # maximum allowed visits per customer
...
x = model.addVars(K_list, N_expanded, N_expanded, vtype=GRB.BINARY, name="x")

# -------------------------------------------------------
# 3.2  q_ij^k: The flow of load carried from vertex i to vertex j by vehicle k
# -------------------------------------------------------
#    Continuous variable.
q = model.addVars(K_list, N_expanded, N_expanded, vtype=GRB.CONTINUOUS, lb=0.0, name="q")

# -------------------------------------------------------
# 3.3  y_i^k: Cumulative charge amount variable 
# -------------------------------------------------------
#    Continuous variable.
#    The current energy level of vehicle k when arriving at vertex i
y = model.addVars(K_e_list, N_expanded, vtype=GRB.CONTINUOUS, lb=0.0, name="y")
...
U = model.addVars(K_list, N_expanded, vtype=GRB.CONTINUOUS, lb=0.0, name="U")
...
# -------------------------------------------------------
# 3.12  Delivered_k^i: the amount of load delivered at customer i
# -------------------------------------------------------
#   Continuous variable
#   Used for tracking deliveries at each customer
delivered = model.addVars(K_list, D_expanded, vtype=GRB.CONTINUOUS, lb=0.0, name="delivered")
...
# -------------------------------------------------------
term1 = r * gp.quicksum(
    h[k] * d_exp[(i, j)] * x[(k, i[0], i[1], j[0], j[1])]
    for k in K_e_list
...
term2 = F * gp.quicksum(
    ...
    ) * lambda_param * x[(k, i[0], i[1], j[0], j[1])]
    for k in K_c_list
    for i in N_expanded for j in N_expanded if i != j
)
...
# -------------------------------------------------------
term3 = F * gp.quicksum(
    phi_param * sigma_param * lambda_param * d_exp[(i, j)] * q[(k, i[0], i[1], j[0], j[1])]
    for k in K_c_list
    for i in N_expanded for j in N_expanded if i != j
)

# -------------------------------------------------------
# 1.4 General Operating Costs + Driver Wage Costs
# -------------------------------------------------------
term4 = gp.quicksum(
    O[k] * d_exp[(i, j)] * x[(k, i[0], i[1], j[0], j[1])]
    for k in K_list
    for i in N_expanded for j in N_expanded if i != j
) + W * gp.quicksum(
    t_exp[(i, j)] * x[(k, i[0], i[1], j[0], j[1])]
    for k in K_list
    for i in N_expanded for j in N_expanded if i != j
)
...
# -------------------------------------------------------
model.addConstrs(
    gp.quicksum(x[(k, depot, 0, j[0], j[1])] for j in N_expanded if j != (depot, 0)) >= used[k]
    for k in K_list
)
...
model.addConstrs(
    gp.quicksum(x[(k, depot, 0, j[0], j[1])] for j in N_expanded if j != (depot, 0)) ==
    gp.quicksum(x[(k, i[0], i[1], depot, 0)] for i in N_expanded if i != (depot, 0))
    for k in K_list
)
...
# -------------------------------------------------------
model.addConstrs(
    (gp.quicksum(x[(k, i[0], i[1], j[0], j[1])] for i in N_expanded if i != j) -
     gp.quicksum(x[(k, j[0], j[1], i[0], i[1])] for i in N_expanded if i != j)) == 0
...
penalty_nodes = {node for node in N_expanded if node[0] not in CS}

for k in K_list:
    # Compute the total travel time using the expanded travel time dictionary t_exp and flattened x keys.
    ...
    model.addConstr(
        T[k] >= travel_time_expr + Tstop * (penalty_arcs_expr - used[k]),
        name=f"TimeLink_{k}"
    )
...
for k in K_list:
    for j in N_expanded:
        if j != (depot, 0):
            model.addConstr(used[k] >= x[(k, depot, 0, j[0], j[1])],
...
# Linking q and k
model.addConstrs(
    (q[(k, i[0], i[1], j[0], j[1])] <= Q[k] * x[(k, i[0], i[1], j[0], j[1])]
...
for i in D:
    model.addConstr(
        gp.quicksum(delivered[(k, i, v)] for k in K_list for v in range(1, Vmax+1)) == p[i],
...
for k in K_list:
    for j in D_expanded:
        model.addConstr(
            gp.quicksum(q[(k, i[0], i[1], j[0], j[1])] for i in N_expanded if i != j) ==
            delivered[(k, j[0], j[1])] + gp.quicksum(q[(k, j[0], j[1], i[0], i[1])] for i in N_expanded if i != j),
...
for k in K_list:
    ...
    model.addConstr(U[(k, depot, 0)] == Q[k], name=f"initial_load_{k}")
    for i in N_expanded:
        for j in N_expanded:
            if i == j:
                continue
            if i == (depot, 0) and j in D_expanded:
                ...
                model.addGenConstrIndicator(
                    x[(k, depot, 0, j[0], j[1])], True,
                    U[(k, j[0], j[1])] == Q[k] - delivered[(k, j[0], j[1])],
                    name=f"dep_depart_{k}_{j}"
                )
            elif j == (depot, 0) and i in D_expanded:
                # Arriving at the depot resets the load to full capacity.
                model.addGenConstrIndicator(
                    x[(k, i[0], i[1], depot, 0)], True,
                    U[(k, depot, 0)] == Q[k],
                    name=f"arrive_dep_{k}_{i}"
                )
            elif i in D_expanded and j in D_expanded:
                # Between customers: subtract the delivery at j from the remaining load at i.
                model.addGenConstrIndicator(
                    x[(k, i[0], i[1], j[0], j[1])], True,
                    U[(k, j[0], j[1])] == U[(k, i[0], i[1])] - delivered[(k, j[0], j[1])],
                    name=f"update_load_{k}_{i}_{j}"
                )
...
for k in K_list:
    for j in D_expanded:
        model.addConstr(
            delivered[(k, j[0], j[1])] <= p[j[0]] * gp.quicksum(x[(k, j[0], j[1], i[0], i[1])] for i in N_expanded if i != j),
...
for k in K_list:
    for j in D_expanded:
        model.addConstr(
            delivered[(k, j[0], j[1])] >= Q[k] / 2 * gp.quicksum(x[(k, j[0], j[1], i[0], i[1])] for i in N_expanded if i != j),
            name=f"min_delivery_{k}_{j}"
        )
...
# -------------------------------------------------------
# For each arc (i,j) used by an HGEV:
for k in K_e_list:
    # At the depot, vehicles start with full charge.
    model.addConstr(y[(k, depot, 0)] == R[k], name=f"initial_charge_{k}")
    for i in N_expanded:
        for j in N_expanded:
            if i == j:
                continue
            ...
            if j == (depot, 0):
                model.addGenConstrIndicator(
                    x[(k, i[0], i[1], depot, 0)], True,
                    y[(k, depot, 0)] == R[k],
                    name=f"recharge_at_{k}_{i}_{(depot, 0)}"
                )
            ...
            # infrastructure network has been installed, then recharge:
                ...
                model.addGenConstrIndicator(
                    HGEV_exp, True,
                    y[(k, j[0], j[1])] == R[k],
                    name=f"recharge_at_CS_{k}_{i}_{j}"
                )
            # Otherwise, the battery level is reduced.
            else:
                model.addGenConstrIndicator(
                    x[(k, i[0], i[1], j[0], j[1])], True,
                    y[(k, j[0], j[1])] == y[(k, i[0], i[1])] - h[k] * d_exp[(i, j)],
...
selected_arcs = [
    (k, (i0, i1), (j0, j1))
    for (k, i0, i1, j0, j1) in x.keys()
    if x[(k, i0, i1, j0, j1)].X > 0.5]
...
#print(f"Fuel efficiency of CFV trucks (mpg): {2.35215 * ((sum(d[(i[0], j[0])] for (k, i, j) in selected_arcs if k in K_c_list)/1000)/((diesel_cost_load + diesel_cost_curb)/F))}")
...
#print(f"Total distance traveled: {sum(d[(i[0], j[0])] for (k, i, j) in selected_arcs)/1000}km")
...
for k in K_list:
    print(f"\nVehicle {k}:")
    # Find all arcs selected by this vehicle: note that x is now indexed as (k, i[0], i[1], j[0], j[1])
    selected_arcs = [
        (i, j) 
        for i in N_expanded 
        for j in N_expanded 
        if i != j and x[(k, i[0], i[1], j[0], j[1])].X > 0.5
    ]
    ...
    for i, j in selected_arcs:
        if j == (depot, 0):
            ...
            print(f"  Arc ({i} -> {j}): Arriving at depot. U[{k}, {j}] = {U[(k, j[0], j[1])].X:.2f} (should be Q[{k}] = {Q[k]:.2f}).")
        elif i == (depot, 0) and j != (depot, 0):
            # When leaving the depot, expected load at j = full capacity minus the delivery at j.
            expected_load = Q[k] - delivered[(k, j[0], j[1])].X
            print(f"  Arc ({i} -> {j}): U[{k}, {j}] = {U[(k, j[0], j[1])].X:.2f}, Delivered[{k}, {j}] = {delivered[(k, j[0], j[1])].X:.2f}, Expected U[{k}, {j}] = {expected_load:.2f}")
        elif i != (depot, 0) and j != (depot, 0):
            # For arcs between customers: expected load at j = load at i minus delivery at j.
            expected_load = U[(k, i[0], i[1])].X - delivered[(k, j[0], j[1])].X
            print(f"  Arc ({i} -> {j}): U[{k}, {i}] = {U[(k, i[0], i[1])].X:.2f}, Delivered[{k}, {j}] = {delivered[(k, j[0], j[1])].X:.2f}, U[{k}, {j}] = {U[(k, j[0], j[1])].X:.2f}, Expected U[{k}, {j}] = {expected_load:.2f}")
...
for k in K_e_list:
    ...
    # Gather all arcs used by vehicle k.
    # Note: x is defined with flattened keys (k, i[0], i[1], j[0], j[1])
    vehicle_arcs = [
        (i, j) 
        for i in N_expanded 
        for j in N_expanded 
        if i != j and x[(k, i[0], i[1], j[0], j[1])].X > 0.5]
    if not vehicle_arcs:
        print(f"Vehicle {k}: No arcs selected.")
        continue
    ...
    print(f"\nVehicle {k} Routes and Battery Levels:")
    for tour in tours:
        route_str = " -> ".join(str(node) for node in tour)
        print(f"  Route: {route_str}")
        for node in tour:
            # Access y using the flattened key: (k, node[0], node[1])
            battery_level = y[(k, node[0], node[1])].X
            if node == (depot, 0):
...
for k in K_list:
    ...
    # Gather all selected arcs for this vehicle.
    # Note: x is defined over (k, i[0], i[1], j[0], j[1])
    vehicle_arcs = [
        (i, j)
        for i in N_expanded 
        for j in N_expanded 
    ...
    while any(i == (depot, 0) for (i, j) in remaining_arcs):
        ...
        while current_node != (depot, 0):
            ...
            current_node = next_arc[1]
        tours.append(tour)
    # Print the tours for this vehicle.
    print(f"\nVehicle {k} Routes:")
